...
function List2({ todo, onBack, handleSelectTodo, setSelectedTodo, list1Name }) {
  ...
  const handleCalendarChange = (e) => {
    ...
    const differenceInDays = Math.ceil(
      (new Date().setHours(0, 0, 0, 0) - selectedDate.setHours(0, 0, 0, 0)) /
        (1000 * 60 * 60 * 24)
    );
  ...
  const handleDelete = () => {
    ...
    axios
      .delete(
        `http://ec2-3-36-117-96.ap-northeast-2.compute.amazonaws.com:3000/api/todos/delete/${todo._id}`
      )
      .then((res) => {
        alert("일정이 삭제되었습니다.");
        onBack(list1Name);
  ...
  const update = () => {
    ...
    if (recurringEvent) {
      ...
      if (dueDateChange !== null) {
        if (dueDateChange >= new Date().toISOString().split("T")[0]) {
    ...
    axios
      .put(
        `http://ec2-3-36-117-96.ap-northeast-2.compute.amazonaws.com:3000/api/todos/update`,
        updateDate
      )
      .then((res) => {
